...
std::vector<std::shared_ptr<Code>> elim_labels(std::vector<std::shared_ptr<Code>> program) {
    ...
    for (auto& code : program) {
        if (std::shared_ptr<DefineLabel> define_label = std::dynamic_pointer_cast<DefineLabel>(code)) {
    ...
    for (auto& code : program) {
        ...
        std::shared_ptr<UseLabel> debuglabel = std::dynamic_pointer_cast<UseLabel>(code);

        location+=4;
        if (std::shared_ptr<Word> word = std::dynamic_pointer_cast<Word>(code)) {
